...
function InvoiceCreatePage() {
  ...
  const formik = useFormik<InvoiceInterface>({
    initialValues: {
      ...
      payment_due_date: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="payment_due_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Payment Due Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.payment_due_date ? new Date(formik.values?.payment_due_date) : null}
              onChange={(value: Date) => formik.setFieldValue('payment_due_date', value)}
            />
          </FormControl>
